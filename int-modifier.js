...
} else if (requestType == "Add Mortgage Company") {
    // Assign relevant variables for logic check.
...
// Else statement (temporary) to make <h3> visible on all requests.
    ...
    if (policyCount > 0) {
        $('body').append("<aside id=\"mydiv\"><h2>Count: " + (policyCount - 15) + " (" + ((policyCount - 15) / 23).toFixed(2) + " Hour(s))</h2></p><button type=\"button\" id=\"copyButton\">Copy</button></aside>");
    } else {
        $('body').append("<aside id=\"mydiv\"><h2>" + requestType + "</h2></p><button type=\"button\" id=\"copyButton\">Copy</button></aside>");
    }
}
...
$(window).load(dragElement(document.getElementById("mydiv")));
