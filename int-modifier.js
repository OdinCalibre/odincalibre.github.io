...
// Else statement (temporary) to make <h3> visible on all requests.
    ...
    if (policyCount > 0) {
        $('body').append("<aside><h2>Count: " + (policyCount - 8) + " (" + ((policyCount - 8) / 23).toFixed(2) + " Hour(s))</h2></p><button type=\"button\" id=\"copyButton\">Copy</button></aside>");
    } else {
        $('body').append("<aside><h2>" + requestType + "</h2></p><button type=\"button\" id=\"copyButton\">Copy</button></aside>");
    }
}

// TEMPORARY - Code to remove all items in queue that are older than 9/21/2022
$("#ajaxFormName li").each(function(index, value) {
    var time = (this.id);
    if (time <= 1433630) {
        $(this).remove();
    }
